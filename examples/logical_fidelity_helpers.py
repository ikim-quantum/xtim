...
def demo_extract_frame() -> None:
    ...
    print()
    print(f"extract_frame(..., minimal=True)")
    ...
    print()
    print(f"extract_frame(..., minimal=False)  — full-readout representative")
...
def demo_fidelity_from_logicals() -> None:
    ...
    # ── Step C: contrast with postselected_logical_error_rate ────────────────
    print()
    print(f"── Step C: contrast with postselected_logical_error_rate ──")
    with _warnings.catch_warnings(record=True) as _w:
        _warnings.simplefilter("always")
...
def demo_partial_support_divergence() -> None:
    ...
    print(f"  (stripped all PAULI_EXPECTATION lines, re-declared only X̄)")
    print()
