...
### Declare what you consume — `xtim.port` (new in 3.0)

When you need more than dets and observables — the per-shot DECISION bits, or
the group-indexed residual record — declare it once and let the port route to
the cheapest engine surface that serves the declaration. Nothing undeclared is
computed:

```python
from xtim.port import Consume, compile as compile_segment

seg = compile_segment(c.text, Consume(dets=True, decisions=True))
res = seg.run(shots=200_000, seed=42)   # stateless draw; reseeds in place
res.dets                                # bool[shots, n_det]
res.decisions                           # bool[shots, n_dec]
```

`Consume(groups=True)` adds the group-indexed record surface (one row per
distinct residual class rather than per shot) and `Consume(residual_law=True)`
adds the compile-time σ-law read model. `Segment.run(shots, seed)` is a
**stateless draw** — nothing carries from one call to the next, by design; see
"THE SEAM CONTRACT" in [`xtim/port.py`](xtim/port.py).
...
- The `v3.x` API is the supported line. `3.0.0` delivers the declared-
  consumption `xtim.port` surface the `2.7.0` notes promised and removes two
  never-publicly-released dead parameters; everything else in the `v2.x` API is
