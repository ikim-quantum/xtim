...
SamplerProgram* compile_sampler_program(const Circuit& user_circuit,
    ...
    // The noiseless reference's records go through the SAME two relabel passes as every shot's,
    // in the SAME order as pack_shot_records: the deterministic invert FIRST, the feedback SECOND.
    // The order is load-bearing: a feedback reads the RECORDED control bit, invert included, so
    // relabeling the reference feedback-first would let it read a pre-invert bit and invert every
    // detector/observable that feedback feeds. (That happened for every circuit with an H: an
    // eliminate_hadamards rebuild absorbs `X c; M c` into `M c` with a stamped invert, so a
    // control record could carry an invert the reference's feedback pass had not yet applied.)
    //
    // 1. The deterministic `!` invert is part of every shot's record (and the noiseless
    //    reference's), so fold it into refmw BEFORE refdet/refobs are computed — the per-shot
    //    XOR-reference parity then cancels it out for any detector/observable that includes an
    //    even count of inverted records, exactly as Stim does. The probabilistic M(p) coin is
    //    per-shot, NOT in the reference.
    if (has_rf)
        for (int j = 0; j < M; ++j)
            if (rec_invert[j]) refmw[j >> 6] ^= 1ull << (j & 63);
    // 2. The detector/observable channels are reference-relative; with feedback the noiseless
