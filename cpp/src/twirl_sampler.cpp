...
struct TwirlRecordSampler::Impl {
    ...
    // Born fast-path (2026-08-08): the abelian warm path has no materialised nf; it reconstructs the
    // post-canon residual prefix into this scratch Pauli (X from v-support, Z from a&2 ⊕ zflip) so
    // emit_decisions's memo key (⟨W_i,P⟩ signs) + materialize_shot's plankey are constructible on
    // the fast path exactly as the structural path built them from nf.prefix.
    Pauli fast_prefix_;
    long fast_born_shots_ = 0;     // diagnostic counter (prof): born shots served by the abelian fast path
    // Reconstruct the memo-key inputs for a born decision on the ABELIAN warm fast path, from the
    // accumulator coordinates (key_amask_s / cz_key / zflip / v_dirty·v_acc / a_dirty·a_acc). Produces
    // the SAME (cur_plan_, cur_prefix_, plankey_) the structural path built from the canonicalized nf
    // (nf.a == key_amask_s, nf.cz == cz_key, nf.prefix.x == v-support, nf.prefix.z == (a&2)⊕zflip).
    // Byte-format of plankey_ is byte-identical to the slow path's serialization (append_words x/z,
    // a&1 bytes, 0xFF, cz pairs) so materialize_shot parses it identically.
    void capture_born_fastpath_(const CachedPlan* fplan) {
        cur_plan_ = fplan;
        const int NW = (n + 63) / 64;
        fast_prefix_ = Pauli(n);
        // prefix.x = v-support (X on qubits with v_acc set).
        for (int q : v_dirty) if (v_acc[(size_t)q]) fast_prefix_.setx(q);
        // prefix.z = (a&2 endpoints) ⊕ (zflip corrections); XOR self-inverse per occurrence.
        for (int q : a_dirty) if (a_acc[(size_t)q] & 2) fast_prefix_.flipz(q);
        for (int q : zflip) fast_prefix_.flipz(q);
        cur_prefix_ = &fast_prefix_;
        // Serialize plankey_ EXACTLY as the structural path (see run(): the "barrier2 Lever 2" block).
        plankey_.clear();
        plankey_.reserve(fast_prefix_.x.size() * 8 + fast_prefix_.z.size() * 8 +
                         (size_t)n + 1 + cz_key.size() * 4);
        ...
        append_words(fast_prefix_.x);
        append_words(fast_prefix_.z);
        for (int q = 0; q < n; ++q)
            plankey_.push_back((uint8_t)((key_amask_s[(size_t)(q >> 6)] >> (q & 63)) & 1));
        plankey_.push_back(0xFF);
    ...
    FramedSuperposition materialize_shot(const uint8_t* plan_key, int n_plan,
                                         ...
                                         const CachedPlan* plan = nullptr) const {
        DiagNormalForm nf;
        ...
        // Born fast-path (2026-08-08): the ShotLaw-except-plane1 is a pure function of (G, a, cz) —
        // exactly the CachedPlan's cached content. When the caller hands us the interned plan for
        // this shot, reuse plan->finalize(G, nf) (cheap: copy cached fields + XOR plane1(prefix))
        // instead of build_shot_law (which re-derives the whole lattice law — ~94 µs/call on the
        // 15-generator H+T group, the dominant born-shot cost). finalize() is the SAME law
        // get_or_build cached: base0 == build_shot_law(identity-prefix).det_signs, and det_signs =
        // base0 ⊕ plane1(P). Byte-identical amps (proven at 5σ + the born A/B routing oracle).
        // QEC_BORN_LAWCHK asserts finalize's collapsed amps == build_shot_law's, per call.
        ShotLaw law = (plan && !plan->fallback) ? plan->finalize(G, nf) : build_shot_law(G, nf);
        if (plan && !plan->fallback && std::getenv("QEC_BORN_LAWCHK")) {
            // Assert the AMPLITUDE-relevant law fields match build_shot_law EXACTLY. det_signs is a
            // σ-space detector base point that never enters twirl_collapse's amps (memo §2) — it may
            // differ between finalize and build_shot_law without affecting born_p1; excluded here.
            ShotLaw ref = build_shot_law(G, nf);
            if (law.r != ref.r || law.kappa != ref.kappa ||
                law.coin_masks != ref.coin_masks || law.kernel_masks != ref.kernel_masks ||
                law.kernel_base != ref.kernel_base || law.kernel_foldable != ref.kernel_foldable ||
                law.kernel_logicals.size() != ref.kernel_logicals.size()) {
                fprintf(stderr, "[BORN LAWCHK FAIL] amp-relevant law fields differ finalize vs build_shot_law\n");
                std::abort();
            }
        }
        int idx = 0;
...
bool TwirlRecordSampler::Impl::run(long shots) {
    ...
    // Born fast-path (2026-08-08): `need_state` is now retention ALONE. Born DECISIONs no longer
    // force the structural per-shot canonicalize/collapse detour — they run on the abelian σ fast
    // path and feed the EXISTING born-probability memo (materialize_shot on a memo miss). This is a
    // COMPILE-TIME routing decision (has_born_dec_ is set once at construction). Retention still
    // needs the materialised per-shot amps ⇒ keeps need_state; born gets its p_j from the memo.
    // Class-(b) genuinely-magic-weighted decisions (m_out = (1∓1/√2)/2) stay correct via the memo's
    // materialize_shot; the win is the σ path + class-(a)/memoized shots no longer force the detour.
    // See the born-decision fast-path scope memo (adaptq docs). Coin RECORD note: the fast path draws
    // the r fair coins from the 64-coins/word reservoir instead of one next_double() word per coin, so
    // the recorded o_rec.coins bit-pattern (the memo key's coins_bits) is a different reservoir draw.
    // The memo authorised a one-time born-coin re-pin for this. MEASURED OUTCOME: the born DECISION
    // stream is nonetheless byte-IDENTICAL to the pre-change structural path — because for a κ=0
    // diagonal plan the collapsed AMPLITUDES (hence born_p1 = p_j) are independent of the r fair
    // coins (fair coins only flip σ / detector signs, never the amp structure). So p_j, and thus the
    // dec_gen-driven decision, is invariant to the coin re-pin: no born byte moved (verified 40k H+T
    // + born5 + 5σ physical invariance). The authorised re-pin did not need to be spent.
    const bool need_state = retain_state;
    ...
    if ((need_state || has_born_dec_) && ppr_tables) {
        // Born fast-path (2026-08-08): the refusal is on (retention OR born) — has_born_dec_ is
        // spelled out explicitly now that need_state no longer subsumes it. CH/PPR born decisions
        // stay refused (no closed-form abelian σ law; the memo's materialize_shot has no PPR path).
    ...
    while (done < shots) {
        ...
        for (int s = 0; s < S; ++s, ++done) {
            ...
            if (e0 == e1) {
                ...
                // Born fast-path (2026-08-08): born decisions are EXCLUDED from this streaming-blob
                // shortcut (it reconstructs channel bits from per-alt rows and never materialises the
                // residual PREFIX, which the born memo key needs). Born diagonal shots fall through to
                // the ACCUMULATOR fast path (~2680) which builds key_amask_s + the residual prefix in
                // accumulator coordinates — the born memo inputs — while still skipping the structural
                // canonicalize/twirl_collapse_record detour.
                if (!born_obs && checked >= sc && !need_state && !has_born_dec_) {
                ...
                if (fplan && !fplan->fallback && fplan->kernel_logicals.empty() &&
                    ...
                    // Born fast-path (2026-08-08): when born decisions are declared, RECORD each
                    // fair coin bit into o_rec.coins — the born memo's coins_bits / materialize_shot
                    // replay need them (κ=0 on the abelian path ⇒ o_rec.coins is exactly the r fair
                    // coins, same SHAPE as the structural path's r fair coins). The recorded VALUES
                    // are a reservoir draw (differ from the old next_double() draw), but p_j is
                    // invariant to the fair coins for κ=0 diagonal plans ⇒ the born decision does not
                    // move (see the need_state comment above).
                    if (has_born_dec_) o_rec.coins.clear();
                    for (size_t k = 0; k < fplan->ch_coin_rows.size(); ++k) {
                        if (coin_left == 0) { coin_res = shot_rng.next(); coin_left = 64; }
                        const bool bit = coin_res & 1; coin_res >>= 1; --coin_left;
                        if (has_born_dec_) o_rec.coins.push_back((uint8_t)bit);
                        if (bit) for (int w2 = 0; w2 < CW; ++w2) bits[w2] ^= fplan->ch_coin_rows[k][w2];
                    }
                    // Born fast-path: capture the memo key inputs (plan id, residual prefix signs,
                    // plankey for a memo miss) from the abelian accumulator coordinates BEFORE the
                    // dirty lists are reset. The reconstructed post-canon nf = (key_amask_s a-mask,
                    // cz_key, prefix.x=v-support, prefix.z=(a&2)⊕zflip) is byte-identical to the
                    // structural path's canonicalized nf — proven by the selfcheck-forced slow path
                    // producing the same born decisions.
                    if (has_born_dec_) {
                        capture_born_fastpath_(fplan);
                    }
                    ...
                    if (has_born_dec_) {
                        // Full born emit: measure the decisions on the memoised collapse, append the
                        // raw born outcomes to the coin record (record-hash key), and hand the sink
                        // the compact plan identity + prefix for record reconstruction.
                        ++fast_born_shots_;
                        const uint8_t* dptr = emit_decisions(bits, nullptr);
                        for (uint8_t b : born_raw_) o_rec.coins.push_back(b);
                        emit_shot(bits, 0, false, nullptr, nullptr, dptr,
                                  o_rec.coins.empty() ? nullptr : o_rec.coins.data(),
                                  (int)o_rec.coins.size(),
                                  plankey_.empty() ? nullptr : plankey_.data(), (int)plankey_.size(),
                                  born_u_.empty() ? nullptr : born_u_.data(), (int)born_u_.size(),
                                  cur_plan_, fast_prefix_.x.data(), fast_prefix_.z.data());
                    } else {
                        emit_shot(bits, 0, false, nullptr, nullptr, emit_decisions(bits, nullptr), nullptr, 0, nullptr, 0, nullptr, 0);
                    }
                    continue;
                ...
                if (plan.fallback || !nf.diagonal_class || !plan.kernel_logicals.empty()) {
                    ...
                    twirl_collapse_record(bare, G, nf, plan, coll_rng, o_rec);
                    // Born fast-path (2026-08-08): a born decision on the κ>0 / fallback branch has no
                    // closed-form abelian σ (the memo materialize_shot handles only κ=0 diagonal
                    // plans), so measure the decisions DIRECTLY on the exact collapsed state — the
                    // same exact-state path the old structural need_state fallback used (dec_state_ =
                    // &fbwork). Forces the exact branch below even absent an obs.
                    const bool born_needs_exact = has_born_dec_ && !o_rec.fallback && !born_obs;
                    if (o_rec.fallback || born_obs || born_needs_exact) {
                        ...
                        exact_cur = true;
                        exact_diag_shot(nf, bits);
                        // Born fast-path: measure the decisions on the exact collapsed state.
                        if (has_born_dec_) dec_state_ = &fbwork;
                    } else {
                        for (int c = 0; c < nchan; ++c)
                    ...
                    // Born fast-path (2026-08-08): this cold/self-check ABELIAN shot feeds the born
                    // memo exactly like the warm fast path — RECORD the r fair coins into o_rec.coins
                    // (κ=0 here ⇒ exactly r coins, same shape as the warm path) and capture the memo
                    // key inputs from nf. Routing-independent by construction: the warm and cold
                    // routes draw the SAME reservoir coins for the SAME plan ⇒ identical born stream.
                    if (has_born_dec_) o_rec.coins.clear();
                    ...
                    // Born fast-path (2026-08-08): capture the memo key inputs for this cold/
                    // self-check ABELIAN shot from the materialised nf (identical to the warm
                    // path's reconstruction from accumulator coords), then route through the
                    // shared born emit below. On a guard_exact born shot measure on the exact state.
                    if (has_born_dec_) {
                        if (guard_exact) {
                            dec_state_ = &fbwork;
                        } else {
    ...
    if (prof && has_born_dec_)
        fprintf(stderr, "[twirl_records PROF born] fast_born_shots=%ld slow_shots=%ld "
                        "born_memo_size=%zu of shots=%ld\n",
                fast_born_shots_, slow_shots, u64_memo_count_, shots);
