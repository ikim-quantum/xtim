...
struct PyFramedSuperposition {
    ...
    // 3.1.0: the overlap VALUE |<self|other>|^2 (global-phase insensitive), the same exact
    // exact_sum_overlap that approx_equal thresholds. Both states must live on the same
    // qubit set (same n); a mismatch is a caller error, refused loudly. Consumers: per-shot
    // Choi-state fidelity estimation (a stabilizer-superposition inner product per shot).
    double overlap(const PyFramedSuperposition& other) const {
        if (state.n() != other.state.n())
            throw std::invalid_argument(
                "FramedSuperposition.overlap: states have different qubit counts (" +
                std::to_string(state.n()) + " vs " + std::to_string(other.state.n()) +
                "); overlaps are defined on one qubit set");
        auto ov = exact_sum_overlap(state, other.state);
        return std::norm(ov);
    }
    ...
    // 3.1.2 (branch frames, docs/branch_frames.md): export the state as
    //   |psi> = sum_i coeff[i] · D_i |ref>,
    // |ref> = the joint +1 eigenstate of the n SIGNED generators G_a = i^{stab_phase[a]}
    // X^{stab_x[a]} Z^{stab_z[a]} (frame row Zrow[a] with (-1)^{eps[a]} folded in — the
    // certified_stabilizers() convention extended to the free rows), and
    //   D_i = Π_{d ascending : sigma_i[d]} Xrow[free[d]]
    // formed LEFT-TO-RIGHT with the engine's Pauli product (pmul_into ≡ Pauli::multiply(acc,b)),
    // i.e. exactly the oW[i] of framed_measure_anticommuting_general. Read-only; O(chi·n) bit
    // copies on the already-materialized state. Every Pauli is over the STATE's n wires (the
    // indices pauli_expectation_x(q) addresses / TwirlSampler.output_wires() returns).
    py::dict branch_frames() const {
        const int N = state.n();
        const int K = state.k();
        const auto& brs = state.entries();
        const int chi = (int)brs.size();
        auto bits2d = [N](int rows) {
            py::array_t<uint8_t> a({rows, N});
            std::memset(a.mutable_data(), 0, (size_t)rows * (size_t)N);
            return a;
        };
        auto fill_row = [N](py::array_t<uint8_t>& ax, py::array_t<uint8_t>& az, int r,
                            const Pauli& p) {
            uint8_t* x = ax.mutable_data() + (size_t)r * (size_t)N;
            uint8_t* z = az.mutable_data() + (size_t)r * (size_t)N;
            for (int q = 0; q < N; ++q) { x[q] = p.xbit(q) ? 1 : 0; z[q] = p.zbit(q) ? 1 : 0; }
        };
        py::array_t<uint8_t> sx = bits2d(N), sz = bits2d(N), dx = bits2d(N), dz = bits2d(N);
        py::array_t<int8_t> sp(N), dp(N);
        py::array_t<uint8_t> eps_a(N);
        for (int a = 0; a < N; ++a) {
            Pauli g = state.U.Zrow[a];
            if (state.eps[a]) g.phase = (g.phase + 2) & 3;      // ×(-1): the signed generator
            fill_row(sx, sz, a, g);
            sp.mutable_data()[a] = (int8_t)(g.phase & 3);
            const Pauli& d = state.U.Xrow[a];
            fill_row(dx, dz, a, d);
            dp.mutable_data()[a] = (int8_t)(d.phase & 3);
            eps_a.mutable_data()[a] = state.eps[a] ? 1 : 0;
        }
        py::array_t<int32_t> fr(K);
        for (int d = 0; d < K; ++d) fr.mutable_data()[d] = (int32_t)state.free[d];
        py::array_t<uint8_t> sig({chi, K});
        if (chi * K) std::memset(sig.mutable_data(), 0, (size_t)chi * (size_t)K);
        py::array_t<std::complex<double>> co(chi);
        py::array_t<uint8_t> bx = bits2d(chi), bz = bits2d(chi);
        py::array_t<int8_t> bp(chi);
        for (int i = 0; i < chi; ++i) {
            Pauli D(N);                                          // identity, phase 0
            for (int d = 0; d < K; ++d) {
                sig.mutable_data()[(size_t)i * (size_t)K + (size_t)d] = brs[i].first[d] ? 1 : 0;
                if (brs[i].first[d]) D = Pauli::multiply(D, state.U.Xrow[state.free[d]]);
            }
            fill_row(bx, bz, i, D);
            bp.mutable_data()[i] = (int8_t)(D.phase & 3);
            co.mutable_data()[i] = brs[i].second;
        }
        py::dict out;
        out["n"] = N;
        out["free"] = fr;
        out["eps"] = eps_a;
        out["stab_x"] = sx; out["stab_z"] = sz; out["stab_phase"] = sp;
        out["destab_x"] = dx; out["destab_z"] = dz; out["destab_phase"] = dp;
        out["sigma"] = sig; out["coeff"] = co;
        out["branch_x"] = bx; out["branch_z"] = bz; out["branch_phase"] = bp;
        return out;
    }
...
struct PyBarrierBuffer {
    ...
    // 3.1.2 frames(): per-buffer memo of plan_structure keyed by the plan's suffix bytes, so a
    // repeated frames() call (or one on a buffer whose plans repeat) is a pure copy (~40 ns/shot)
    // instead of one plan_structure per distinct plan per call (review MINOR-5). Read-only cache:
    // nothing on the sampling path reads it.
    mutable std::map<std::string, TwirlRecordSampler::PlanStructure> frames_plan_memo_;
    ...
    // 3.1.2: shot i's branch-frame export (docs/branch_frames.md) == materialize(i).branch_frames().
    py::dict branch_frames(int i) const { return materialize(i).branch_frames(); }
    ...
    // ── 3.1.2 "frames at the port" (docs/branch_frames.md §7) ──────────────────────────────
    // The per-shot residual PREFIX P (the shot's exact Pauli frame relative to the noiseless
    // reference: residual_normal_form splits the composed end-of-circuit residual into P times a
    // canonical diagonal tail C = S^a·CZ, and the retained state is P·C·|collapsed>) is ALREADY
    // stored per retained shot — compact store: prefix_words_ (x words then z words, PNW_ each);
    // legacy store: the first 16*PNW bytes of the plan key. This exports it, plus the per-shot
    // dense plan id and, per DISTINCT plan, its a-mask / cz list / r / kappa — straight copies of
    // stored data and one plan_structure per distinct plan (no state replay anywhere).
    //   prefix_words : uint64[shots, 2*pnw]   word w bit b <-> wire q = 64*w + b; words [0,pnw)
    //                                          are the X part, [pnw, 2*pnw) the Z part (LE bit order,
    //                                          the sampler's own Pauli word layout). Row all-zero
    //                                          for the identity prefix (clean shots).
    //   plan_id      : uint32[shots]           0 = identity plan (no residual content); k >= 1
    //                                          indexes the per-plan arrays below.
    //   a            : uint8[n_plans+1, n]     S-layer mask per plan (row 0 = identity plan, zeros)
    //   cz           : list[int32[m_k, 2]]     CZ pairs per plan (index k)
    //   r, kappa     : int32[n_plans+1]        fair-coin count / kernel-chain length per plan
    //   n, pnw, shots, n_plans, compact
    // kappa > 0 plans: the collapsed sector then depends on the recorded kernel-chain outcome
    // bits and is NOT a single frame coset, so the frame algebra of §7 does not apply. Default
    // allow_kappa=false REFUSES loudly (ValueError naming the plan ids) — never a silent
    // materialization; allow_kappa=true returns the data and leaves the rule to the caller.
    // Read-only; the sampling path and the stored records are untouched (byte identity by
    // construction: no code on the run() path changes).
    py::dict frames(bool allow_kappa = false) const {
        if (!sampler_)
            throw std::runtime_error(
                "BarrierBuffer.frames: the source sampler is no longer available "
                "(export the frames before re-sampling / destroying the sampler)");
        // N (deferred wire count) from the identity plan's structure — the same source
        // plan_structure() uses; the identity law is memoized by the sampler.
        const TwirlRecordSampler::PlanStructure ps0 = sampler_->plan_structure(nullptr, 0);
        const int N = (int)ps0.nf.a.size();
        const int PNW = (N + 63) / 64;
        py::array_t<uint64_t> words(std::vector<py::ssize_t>{(py::ssize_t)nshots_, (py::ssize_t)(2 * PNW)});
        std::memset(words.mutable_data(), 0, (size_t)nshots_ * (size_t)(2 * PNW) * sizeof(uint64_t));
        py::array_t<uint32_t> pid(nshots_);
        uint64_t* wd = words.mutable_data();
        uint32_t* pd = pid.mutable_data();
        // Per distinct plan: its suffix bytes (a || 0xFF || cz), dense id, in first-seen order.
        std::vector<std::string> suffixes;              // index k-1 -> suffix of plan id k
        if (compact_) {
            if (PNW != PNW_)
                throw std::runtime_error("BarrierBuffer.frames: prefix word width mismatch (store " +
                                         std::to_string(PNW_) + ", n " + std::to_string(N) + ")");
            for (int i = 0; i < nshots_; ++i) {
                pd[i] = plan_id_[(size_t)i];
                if (pd[i])
                    std::memcpy(wd + (size_t)i * (size_t)(2 * PNW),
                                prefix_words_.data() + (size_t)i * (size_t)(2 * PNW_),
                                (size_t)(2 * PNW) * sizeof(uint64_t));
            }
            const int P = (int)(suffix_off_.size() - 1);
            suffixes.reserve((size_t)P);
            for (int k = 1; k <= P; ++k)
                suffixes.emplace_back(suffix_data_.begin() + suffix_off_[(size_t)k - 1],
                                      suffix_data_.begin() + suffix_off_[(size_t)k]);
        } else {
            std::map<std::string, uint32_t> intern;
            for (int i = 0; i < nshots_; ++i) {
                const int pl = pk_len(i);
                if (pl == 0) { pd[i] = 0; continue; }
                if (pl < 16 * PNW)
                    throw std::runtime_error("BarrierBuffer.frames: plan key shorter than the prefix words at shot " +
                                             std::to_string(i));
                const uint8_t* pk = pk_ptr(i);
                for (int w = 0; w < 2 * PNW; ++w) {
                    uint64_t v = 0;
                    for (int b = 0; b < 8; ++b) v |= ((uint64_t)pk[w * 8 + b]) << (b * 8);
                    wd[(size_t)i * (size_t)(2 * PNW) + (size_t)w] = v;
                }
                std::string suf((const char*)pk + 16 * PNW, (const char*)pk + pl);
                auto it = intern.find(suf);
                if (it == intern.end()) {
                    it = intern.emplace(suf, (uint32_t)suffixes.size() + 1).first;
                    suffixes.push_back(suf);
                }
                pd[i] = it->second;
            }
        }
        const int P = (int)suffixes.size();
        py::array_t<uint8_t> amask(std::vector<py::ssize_t>{(py::ssize_t)(P + 1), (py::ssize_t)N});
        std::memset(amask.mutable_data(), 0, (size_t)(P + 1) * (size_t)N);
        py::array_t<int32_t> rr(P + 1), kk(P + 1), fb(P + 1);
        rr.mutable_data()[0] = (int32_t)ps0.law.r; kk.mutable_data()[0] = (int32_t)ps0.law.kappa;
        fb.mutable_data()[0] = ps0.law.fallback ? 1 : 0;
        py::list czs;
        {
            py::array_t<int32_t> cz0(std::vector<py::ssize_t>{0, 2});
            czs.append(cz0);
        }
        std::vector<int> bad;
        std::string full;
        for (int k = 1; k <= P; ++k) {
            // full legacy plan bytes with an IDENTITY prefix: 16*PNW zero bytes || suffix.
            // plan_structure is memoized per suffix on this buffer (review MINOR-5).
            const std::string& suf = suffixes[(size_t)k - 1];
            auto memo = frames_plan_memo_.find(suf);
            if (memo == frames_plan_memo_.end()) {
                full.assign((size_t)(16 * PNW), '\0');
                full += suf;
                memo = frames_plan_memo_.emplace(
                    suf, sampler_->plan_structure(reinterpret_cast<const uint8_t*>(full.data()),
                                                  (int)full.size())).first;
            }
            const TwirlRecordSampler::PlanStructure& ps = memo->second;
            fb.mutable_data()[k] = ps.law.fallback ? 1 : 0;
            uint8_t* row = amask.mutable_data() + (size_t)k * (size_t)N;
            for (int q = 0; q < N && q < (int)ps.nf.a.size(); ++q) row[q] = ps.nf.a[(size_t)q] & 1;
            rr.mutable_data()[k] = (int32_t)ps.law.r;
            kk.mutable_data()[k] = (int32_t)ps.law.kappa;
            py::array_t<int32_t> cz(std::vector<py::ssize_t>{(py::ssize_t)ps.nf.cz.size(), 2});
            for (size_t e = 0; e < ps.nf.cz.size(); ++e) {
                cz.mutable_data()[2 * e] = (int32_t)ps.nf.cz[e].first;
                cz.mutable_data()[2 * e + 1] = (int32_t)ps.nf.cz[e].second;
            }
            czs.append(cz);
            if (ps.law.kappa > 0 || ps.law.fallback) bad.push_back(k);
        }
        if (!allow_kappa && !bad.empty()) {
            std::string ids;
            for (size_t j = 0; j < bad.size() && j < 10; ++j) ids += (j ? "," : "") + std::to_string(bad[j]);
            throw std::invalid_argument(
                "BarrierBuffer.frames: " + std::to_string(bad.size()) + " plan(s) with kappa > 0 or a "
                "fallback law (plan ids " + ids + (bad.size() > 10 ? ",..." : "") + ") — their collapsed "
                "sector depends on the recorded kernel-chain outcomes and is not a single frame coset; "
                "the frame algebra does not apply. Materialize those shots explicitly, or pass "
                "allow_kappa=True and route them yourself. Never silently materialized.");
        }
        py::dict out;
        out["n"] = N; out["pnw"] = PNW; out["shots"] = nshots_; out["n_plans"] = P;
        out["compact"] = compact_;
        out["prefix_words"] = words;
        out["plan_id"] = pid;
        out["a"] = amask;
        out["cz"] = czs;
        out["r"] = rr;
        out["kappa"] = kk;
        out["fallback"] = fb;         // int32[n_plans+1]: 1 = fallback law (refused like kappa > 0)
        return out;
    }
...
// ── The normalization preset shared by every twirl-path entry point ─────────────────────────
// The TwirlSampler family builds NO FeedbackPlan (unlike cpp/src/sampler.cpp, which normalizes
// KeepCoherent and then relabels), so a ControlledPauli that survived coherentization would have
// no exact home here — `Strip`, the policy through 3.1.0, silently DELETED it from the retained
// state and the record algebra. `coherentize_all` rewrites EVERY feedback into coherent gates (or
// throws on a malformed record), so nothing can survive to be stripped or rejected; `Reject`
// states the intent — a survivor would be an engine bug, never a feature to strip.
static NormalizeResult normalize_for_twirl(const Circuit& user) {
    NormalizePolicy pol;
...
public:
    ...
    PyTwirlSampler(const std::string& text, double p_factor, const std::string& disk_cache,
        ...
        {
            py::gil_scoped_release rel;
            // The bench driver's setup recipe (framed_bench.cpp main()) with the twirl
            // normalization preset: parse → normalize_for_twirl → build_bare_state →
            // FramedSuperposition::from_css → terminal reads.
            ...
            if (!ps.ok()) {
                ...
                NormalizeResult nr = normalize_for_twirl(ps.circuit);
...
PYBIND11_MODULE(_xtim, m) {
    ...
    py::class_<PyFramedSuperposition>(m, "FramedSuperposition",
        ...
        .def("overlap", &PyFramedSuperposition::overlap, py::arg("other"),
             "3.1.0: return |<self|other>|^2 exactly (global-phase insensitive) — the value "
             "approx_equal thresholds. Both states must have the same qubit count "
             "(ValueError otherwise). Per-shot Choi-state fidelity estimation uses it.")
        ...
        .def("branch_frames", &PyFramedSuperposition::branch_frames,
             "3.1.2: export the state as |psi> = sum_i coeff[i] D_i |ref> — dict of numpy arrays: "
             "n, free, eps, stab_x/stab_z/stab_phase (signed generators G_a of |ref>), "
             "destab_x/destab_z/destab_phase (d_a), sigma, coeff, branch_x/branch_z/branch_phase "
             "(D_i = prod_{d asc: sigma_i[d]} d_{free[d]}, engine Pauli phases). Read-only. "
             "Contract + consumer algebra: docs/branch_frames.md and xtim.frames.")
    ...
    py::class_<PyBarrierBuffer>(m, "BarrierBuffer",
        ...
        .def("branch_frames", &PyBarrierBuffer::branch_frames, py::arg("i"),
             "3.1.2: materialize(i).branch_frames() — shot i's state as (signed generators, "
             "destabilizers, branch Paulis D_i, amplitudes c_i) with |psi> = sum_i c_i D_i |ref>; "
             "see docs/branch_frames.md for the contract.")
        ...
        .def("frames", &PyBarrierBuffer::frames, py::arg("allow_kappa") = false,
             "3.1.2 (frames at the port, docs/branch_frames.md §7): per retained shot the stored "
             "residual PREFIX P (the shot's exact Pauli frame relative to the noiseless reference) as "
             "prefix_words uint64[shots, 2*pnw] (X words then Z words; bit b of word w <-> wire 64*w+b), "
             "plan_id uint32[shots] (0 = identity plan), and per distinct plan (index = plan id) its "
             "a-mask uint8[n_plans+1, n], cz list, r and kappa int32[n_plans+1]; plus n, pnw, shots, "
             "n_plans, compact. Straight copies of stored data + one plan_structure per distinct plan; "
             "NO state replay. Refuses (ValueError, naming the plan ids) when any plan has kappa > 0 "
             "or a fallback law unless allow_kappa=True — never a silent materialization.")
