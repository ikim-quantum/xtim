...
Self-contained (no data files): run as `python smoke_test.py`.

NOT run automatically today.  It was written as the cibuildwheel test-command,
but CI wheels are a standing-SKIPPED decision (RELEASING.md), `.github/` is
excluded from the public tree, and wheels are built locally — so no wheel is
exercised by this file unless someone runs it by hand.  Claim corrected
2026-08-30 (claims audit, slice D): it previously read "every wheel is
exercised on its target platform", which has not been true since CI wheels were
dropped.  Worth running by hand against a freshly built wheel; the release
checklist's step 6 (suite against the INSTALLED package) is the automated
equivalent.
"""
import numpy as np
