...
def test_dem_with_reject_cultivation():
    ...
    # The packaged copy (xtim/_examples) rather than the repo-root examples/
    # directory, which is NOT shipped in the wheel -- against an installed
    # package the old path always skipped.  Falls back to the repo copy so a
    # source checkout without the package data still works.
    path = Path(xtim.__file__).resolve().parent / "_examples" / "cultivation_d3_faithful.stim"
    if not path.is_file():
        path = (Path(__file__).resolve().parent.parent / "examples"
                / "cultivation_d3_faithful.stim")
    if not path.is_file():
        pytest.skip("cultivation_d3_faithful.stim not bundled (monorepo run)")
